...
from mydesignmodel.yzy_model.CMSINet_240527.CT_Decoder import CT_decoder
from mydesignmodel.yzy_model.CMSINet_240527.PHM import PHM
from mydesignmodel.yzy_model.CMSINet_240527.OFF import OFF
from mydesignmodel.yzy_model.CMSINet_240527.Last_filter import apply_frequency_filter
...
class Fusion(nn.Module):
    def __init__(self,num_class=41,embed_dims=[64, 128, 320, 512]):
        ...
        self.rde_layer1 = PHM(channel=self.channels[1])  # 128
        self.rde_layer2 = PHM(channel=self.channels[2])  # 320
        self.rde_layer3 = PHM(channel=self.channels[3])  # 512

        self.OFFs = nn.ModuleList([
            OFF(dim=embed_dims[0], reduction=64).cuda(),
            OFF(dim=embed_dims[1], reduction=128).cuda(),
            OFF(dim=embed_dims[2], reduction=320).cuda(),
            OFF(dim=embed_dims[3], reduction=512).cuda()])
